  ...
  const placeOrder = async (event) => {
    ...
    food_list.map((item) => {
      if (cartItems[item._id] > 0) {
        let itemInfo = item;
        itemInfo["quantity"] = cartItems[item._id];  // use to quantity of each item
    ...
    if (response.data.success) {
      ...
      window.location.replace(session_url); // it redirects to new page and removes current page from browser history so user cant go back
    }
    else {
      alert("Error");
    }
